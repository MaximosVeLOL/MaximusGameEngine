...
void World::AddObject(MaxObject* object) {
	if (mObjectCount >= OBJECT_LIMIT) {
		ERROR("We are above the object limit!");
	}
	object->mObjectID = mObjectCount;
...
std::vector<MaxObject *> World::uGetObjectsInView() {
	...
	for (ushort i = 0; i < mObjectCount; i++) {
		MaxObject& cur = *mObjects[i];
		if (cur.mTransform.position.x >= mCameraPosition.x && cur.mTransform.position.x <= mCameraPosition.x + gInstance->mWidth && cur.mTransform.position.y >= mCameraPosition.y && cur.mTransform.position.y <= mCameraPosition.y + gInstance->mHeight
			|| cur.mTransform.position.x + cur.mTransform.width >= mCameraPosition.x && cur.mTransform.position.x + cur.mTransform.width <= mCameraPosition.x + gInstance->mWidth && cur.mTransform.position.y + cur.mTransform.height >= mCameraPosition.y && cur.mTransform.position.y + cur.mTransform.height <= mCameraPosition.y + gInstance->mHeight) returnValue.push_back(&cur);
	}
	return returnValue;
}
